...
int    picoshell(char **cmds[])
{
	...
	while (cmds[i])
	{
		...
		if (pid == 0)
		{
			...
			if (last_fd != -1) // 前回のfdがある時のみ、stdinをlast_fdにする
			{
			...
			if(cmds[i + 1]) // 次にpipeがある場合のみfdを変える
			{
				close(fd[0]);
			...
			execvp(cmds[i][0], cmds[i]); // 実行

			exit(1); // 返ってきたときはエラーなので1
		}

		if(last_fd != -1)
			close(last_fd); // 実質ここがclose(fd[0]) 毎回次のターンで閉じる
	...
	while (wait(&status) != -1) // -1: 待つべき子がもういない
	{
